...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkQuery = "SELECT * FROM lunch WHERE name = '$name'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $sql = "INSERT INTO lunch (id, name, mrp, description, image) VALUES (NULL, '$name', '$mrp', '$description', '$file')";

        if (mysqli_query($conn, $sql)) {
            header("Location:addlunch.html"); // Redirect to the same page or another appropriate location
            exit();
        } else {
