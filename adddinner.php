...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkQuery = "SELECT * FROM dinner WHERE name = '$name'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $sql = "INSERT INTO dinner (id, name, mrp, description, image) VALUES (NULL, '$name', '$mrp', '$description', '$file')";

        if (mysqli_query($conn, $sql)) {
            header("Location:adddinner.html"); // Redirect to the same page or another appropriate location
            exit();
        } else {
